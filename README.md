# genpark-delegated-agentic-checkout-guardrail-skill
...
**genpark-delegated-agentic-checkout-guardrail-skill** is a deterministic, zero-dependency Python skill engineered for autonomous AI workflows, multi-agent orchestration, and production deployments.

> **Executive Capability**: Autonomous merchant checkout token & velocity limit guardrail (Agentcard / Stripe)
...
result = client.evaluate_checkout_guardrail()
print(result)
```
...
```json
{
  "mcpServers": {
    "genpark-delegated-agentic-checkout-guardrail-skill": {
      "command": "python",
      "args": ["/path/to/genpark-delegated-agentic-checkout-guardrail-skill/mcp_server.py"]
    }
  }
}
```
